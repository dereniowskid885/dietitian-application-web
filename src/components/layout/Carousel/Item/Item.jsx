...
function Item({ data, page }) {
  ...
  const hideDialog = () => {
    if (showDialog) toggleDialog();
  };

  return (
    <Link className="item" to={`/${page}/${data.id}`} onClick={hideDialog}>
      <div className="item__container">
        {data.img && <img src={data.img} alt="blog carousel item" />}
        <div className="item__content">
          {data.title && <div className="item__title">{data.title}</div>}
          {data.date && <div className="item__date">{data.date}</div>}
